    ...
    app.service('ProjectDeployerDeploymentSyncHelper', function(DataikuAPI, FutureWatcher) {
        ...
        function start(parentScope, deploymentSettings, startingStepName, endingStepName) {
            ...
            function onStepComplete(results) {
                ...
                deployment.futureResponse = null;
                if(deployment.currentStep.name === DEPLOY_STEPS.ACTIVATE_BUNDLE) {
                    deployment.deploymentHookExecutionStatus = result.deploymentHookExecutionStatus;
                }
            ...
            function onStepUpdate(response) {
                ...
                if(deployment.currentStep.name === DEPLOY_STEPS.ACTIVATE_BUNDLE && response.progress && response.progress.states) {
                    deployment.deploymentHookExecutionStatus = response.progress.deploymentHookExecutionStatus;
                }
                $scope.$emit('projectDeploymentUpdate', deployment);
            }
